...
public class HttpServerVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) throws Exception {
        ...
        router.get("/sayhi2/:name").handler(this::sayHiHandler2);
    ...
    private void sayHiHandler2(RoutingContext context) {
        String name = context.request().getParam("name");

        JsonObject _json = new JsonObject();
        _json.put("name", name);
        DeliveryOptions _deliveryOptions = new DeliveryOptions();
        _deliveryOptions.addHeader("action", "sayHi");
        vertx.eventBus().<JsonObject>send(CONFIG_WIKIDB_QUEUE, _json, _deliveryOptions, res -> {
            if (res.failed()) {
                context.response().putHeader("Content-Type", "text/html;charset=utf8");
                context.response().end(res.cause().getMessage());
            } else {
                JsonObject payLoad = res.result().body();
                String msg = payLoad.getString("msg", "无消息");
                context.response().putHeader("Content-Type", "text/html;charset=utf8");
                context.response().end("直接向eventBus发送消息:"+msg);
            }
        });
